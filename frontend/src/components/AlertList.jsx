    ...
    <div className="card h-full flex flex-col">
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <div className="flex gap-0.5 border border-sentinel-700 rounded-md overflow-hidden">
          {FILTER_TABS.map(tab => {
            const count = tab === 'Unacknowledged' ? unacked.length : alerts.length
            return (
              <button
                key={tab}
              ...
              >
                {tab}
                <span className={`text-xs px-1 rounded-full leading-tight ${
                  filter === tab ? 'bg-sentinel-900/30 text-sentinel-900' : 'bg-sentinel-700 text-gray-300'
                }`}>
                  {count}
                </span>
              </button>
